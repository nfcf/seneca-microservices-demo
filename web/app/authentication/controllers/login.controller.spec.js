describe('Login controller', function () {
    ...
    beforeEach(inject(function (_$controller_, _$q_, _$rootScope_, _$location_, _Authentication_, _$httpBackend_) {
        ...
        _$httpBackend_.when('GET', 'resources/locale-en.json').respond({ HEADER: 'Header' });
        _$httpBackend_.when('GET', 'resources/locale-pt.json').respond({ HEADER: 'Header' });

        vm = $controller('LoginController', { $scope: scope, $location: $location, Authentication: Authentication });
    }));
    ...
    describe('.login()', function () {
        ...
        it('should login user if valid credentials', function () {
            spyOn(Authentication, "login").and.callFake(function () {
                ...
                deferred.resolve({ data: { auth_token: 'dummy' } });
            ...
            expect(Authentication.setCookiesAuthToken).toHaveBeenCalledWith({ auth_token: 'dummy' });
        ...
        it('should fail to login user if invalid credentials', function () {
            spyOn(Authentication, "login").and.callFake(function () {
                var deferred = $q.defer();
                deferred.reject({ status: 500, statusText: 'error' });
                return deferred.promise;
            });
